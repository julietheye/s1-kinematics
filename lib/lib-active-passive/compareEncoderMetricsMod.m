function [winners,model_pairs] = compareEncoderMetricsMod(avgNeurEval,neurEval,columnNames,params)
...
%     neuron_ids = unique(metric_table.signalID,'rows');
    neuron_ids = [1:size(avgNeurEval,1)];
    ...
    winners = cell(size(model_pairs,1),size(neuron_ids,2));
    for neuronnum = 1:size(neuron_ids,2)
%         [~,neuron_eval] = getNTidx(metric_table,'signalID',neuron_ids(neuronnum,:));
        %get 100 points for curent unit
        neuron_eval = zeros(size(neurEval,1)/size(avgNeurEval,1),size(neurEval,2));
        for rep=1:size(neurEval,1)/size(avgNeurEval,1)
            neuron_eval(rep,:) = neurEval(neuronnum+size(neuron_ids,2)*(rep-1),:);
        end
...
function winner = test_model_pair(neuron_eval,columnNames,model_pair,params)
    ...
    assert(size(neuron_eval,1)==num_folds*num_repeats,'Number of samples does not match the cross-validation parameters!')
...
%     diffstat = neuron_eval.(strcat(model_pair{2},postfix))-neuron_eval.(strcat(model_pair{1},postfix));
    idx1 = strcmp(columnNames,strcat(model_pair{1},postfix));
    idx2 = strcmp(columnNames,strcat(model_pair{2},postfix));
    diffstat = neuron_eval(:,idx2)-neuron_eval(:,idx1);
